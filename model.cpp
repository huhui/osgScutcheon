...
NCSEModel::Model::Model(QObject *parent) : QObject(parent),
...
NCSEModel::Model::~Model()
{

}


void NCSEModel::Model::addModelNode(QString modelPath, QString modelName)
{
...
void NCSEModel::Model::createScutcheon()
{
    if(!m_pModelNode)
        return;
...
void NCSEModel::Model::hideScutcheon()
{
    if(!m_pScutcheon)
        return;
...
void NCSEModel::Model::showScutcheon()
{
    if(!m_pScutcheon)
        return;
...
void NCSEModel::Model::addModelScutcheonCallback()
{
    ...
    if(!m_pModelScutcheonCallback)
    {
        ...
        m_pModelScutcheonCallback = new NCSEModel::ModelScutcheonCallback(1);//1表示第1个标牌菜单需要更新文字
